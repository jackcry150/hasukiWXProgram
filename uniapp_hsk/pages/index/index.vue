	...
	<view class="analytics-page home">
		...
		<view class="page-shell" :style="{ paddingTop: `${statusBarHeight + 12}px` }">
			...
			<view class="section-head">
				...
				<view class="section-link" @click="goToProductList">
					<text>查看全部</text>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		async handleAddToCart(product) {
			...
			const colorOptions = Array.isArray(product.version)
				? product.version.filter(item => String(item || '').trim())
				: String(product.version || '').split(',').map(item => item.trim()).filter(Boolean)
